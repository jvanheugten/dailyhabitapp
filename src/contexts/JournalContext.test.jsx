...
test('loadEntry returns null for a date with no entry', async () => {
  ...
  let entry
  await act(async () => {
    entry = await result.current.loadEntry('2026-06-08')
  })
...
test('saveEntry creates a new entry', async () => {
  const { result } = renderHook(() => useJournal(), { wrapper })
  await act(async () => {
    await result.current.saveEntry('2026-06-08', 'Good day')
  })
  let entry
  await act(async () => {
    entry = await result.current.loadEntry('2026-06-08')
...
test('saveEntry updates existing entry (upsert)', async () => {
  const { result } = renderHook(() => useJournal(), { wrapper })
  await act(async () => {
    await result.current.saveEntry('2026-06-08', 'First')
  })
  await act(async () => {
    await result.current.saveEntry('2026-06-08', 'Updated')
  })
...
test('getAllEntries returns entries newest first', async () => {
  ...
  let entries
  await act(async () => {
    entries = await result.current.getAllEntries()
  })
